...
from PySide2.QtWidgets import QGridLayout, QLabel, QComboBox, QPushButton
...
class SegmentPreset:
    def __init__(self):
        ...
        )  # keep window on top so it doesn't get coverd by the houdini interface
    ...
    def initUI(self, m):
        ...
        main_grid.addWidget(main_label, 0, 0, rowSpan=5)
        ...
        main_grid.addWidget(self.dropdown, 0, 1, rowSpan=3)

        submit = QPushButton("Submit")
        submit.clicked.connect(self.act)

        main_grid.addWidget(submit, 3, 1, rowSpan=2)
        m.show()

    def act(self):
        # make sure a node is selected before trying to run
        if len(self.nodes) > 0:
            child = self.nodes[0].createOutputNode(
                "DICOM_RangeSegement", "Segment_Tissue"
            )
            child.setParms(
                {"useDefault": 1, "segmentType": self.dropdown.currentIndex()}
            )
